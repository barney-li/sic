...
def model():
    ...
    with tf.name_scope('trn_cost'):
        trn_cost = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=trn_acc_logits_in, labels=trn_acc_labels_in))
        tf.summary.scalar('trn_cost', trn_cost)
    with tf.name_scope('tst_cost'):
        tst_cost = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=tst_acc_logits_in, labels=tst_acc_labels_in))
    ...
    with tf.name_scope('trn_acc'):
        trn_acc = tf.reduce_mean(tf.cast(tf.equal(trn_acc_labels_in, tf.cast(trn_acc_logits_in>0.5, tf.float32)), tf.float32))
        tf.summary.scalar("trn_acc", trn_acc)
    with tf.name_scope('tst_acc'):
        tst_acc = tf.reduce_mean(tf.cast(tf.equal(tst_acc_labels_in, tf.cast(tst_acc_logits_in>0.5, tf.float32)), tf.float32))
